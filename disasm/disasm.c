...
static void DecodeRG(int index,int datasize,int type) {
  ...
  if (mode>=DISASM_FILE) {
    strcpy(name,(const char *)regname[sizeindex][index]);
  ...
  // Next possibility: 32-bit address with SIB byte.
    ...
    *s='\0';
    if (c==0x04 && (sib & 0x07)==0x05) {
      ...
      j=sib & 0x07;
      if (mode>=DISASM_FILE) {
        strcpy(s,(const char *)regname[2][j]);
        seg=addr32[j].defseg;
      };
    };
...
static void DecodeSO(void) {
  ...
  da->indexed=1;
  Memadr(SEG_DS,(const char *)regname[addrsize==2?1:2][REG_ESI],0L,datasize);
};
...
static void DecodeDE(void) {
  ...
  Memadr(SEG_DS,(const char *)regname[addrsize==2?1:2][REG_EDI],0L,datasize);
  ...
  if (pd->mask==0) {                   // Command not found
    ...
    if (mode>=DISASM_FILE) {
      ...
      if (mnemosize!=0) {
        ...
        strcpy(name,(const char *)pd->name);
        for (i=0; name[i]!='\0'; i++) {
