...
static void Scanasm(int mode) {
  ...
  if (isalpha(*asmcmd) || *asmcmd=='_' || *asmcmd=='@') {
    ...
    for (j=0; j<=8; j++) {             // j==8 means "any register"
      if (strcmp(s,(const char *)regname[0][j])!=0) continue;
      idata=j; scan=SCAN_REG8;         // 8-bit register
      return; };
    for (j=0; j<=8; j++) {
      if (strcmp(s,(const char *)regname[1][j])!=0) continue;
      idata=j; scan=SCAN_REG16;        // 16-bit register
      return; };
    for (j=0; j<=8; j++) {
      if (strcmp(s,(const char *)regname[2][j])!=0) continue;
      idata=j; scan=SCAN_REG32;        // 32-bit register
      return; };
    for (j=0; j<6; j++) {
      if (strcmp(s,(const char *)segname[j])!=0) continue;
    ...
    for (j=0; j<=8; j++) {
      if (strcmp(s,(const char *)fpuname[j])!=0) continue;
      idata=j; scan=SCAN_FPU;          // FPU register (alternative coding)
      return; };
    for (j=0; j<=8; j++) {
      if (strcmp(s,(const char *)mmxname[j])!=0) continue;
      idata=j; scan=SCAN_MMX;          // MMX register
      return; };
    for (j=0; j<=8; j++) {
      if (strcmp(s,(const char *)crname[j])!=0) continue;
      idata=j; scan=SCAN_CR;           // Control register
      return; };
    for (j=0; j<=8; j++) {
      if (strcmp(s,(const char *)drname[j])!=0) continue;
      idata=j; scan=SCAN_DR;           // Debug register
      return; };
    for (j=0; j<sizeof(sizename)/sizeof(sizename[0]); j++) {
      if (strcmp(s,(const char *)sizename[j])!=0) continue;
  ...
  for (pd=cmddata; pd->mask!=0; pd++) {
    if (pd->name[0]=='&') {            // Mnemonic depends on operand size
      ...
      while (1) {
        ...
        if (j-k==namelen && strnicmp(name,(const char *)pd->name+k,namelen)==0) break;
