...
static void CG_G2PlayerAngles( centity_t *cent, vec3_t legs[3], vec3_t legsAngles)
{
	...
	//RoboPhred: this check is duplicate from in CG_Player
...
void CG_AddSaberBlade( centity_t *cent, centity_t *scent, refEntity_t *saber, int renderfx, int modelIndex, int saberNum, int bladeNum, vec3_t origin, vec3_t angles, qboolean fromSaber, qboolean dontDraw)
{
	...
	//Lugormod
	//client->saber[saberNum].blade[bladeNum].lengthMax = client->saber[saberNum].blade[bladeNum].unscaledLengthMax * cent->currentState.iModelScale / 100;
...
JustDoIt:
	...
	CG_DoSaber( org_, axis_[0], saberLen, client->saber[saberNum].blade[bladeNum].lengthMax, 
		client->saber[saberNum].blade[bladeNum].radius,
...
stillDoSaber:
					...
					while (m < ci->saber[0].numBlades)
					{
						...
						if (tagBolt == -1)
						{
							...
							{ //guess this is an 0ldsk3wl saber
								...
								if (tagBolt == -1)
								{
									//RoboPhred: ignore this for sword entities
									//assert(0);
								}
								break;
							}
	...
	{
		if (cent->currentState.eFlags2&EF2_CANSEE //Lugormod
	...
	{ //I guess go ahead and glow your own client too in a duel
		...
		{ //adjust the glow by how far away you are from your dueling partner
			...
			if (duelEnt)
			{
				...
				legs.shaderRGBA[0] = maximum(255-subLen/4,1);
				legs.shaderRGBA[1] = maximum(255-subLen/4,1);
				legs.shaderRGBA[2] = maximum(255-subLen/4,1);
				...
				legs.shaderRGBA[0] = maximum(savRGBA[0]-subLen/8,1);
				legs.shaderRGBA[1] = maximum(savRGBA[1]-subLen/8,1);
				legs.shaderRGBA[2] = maximum(savRGBA[2]-subLen/8,1);
				}
