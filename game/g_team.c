...
void Team_SetFlagStatus( int team, flagStatus_t status ) {
	...
	if( modified ) {
		...
		if( g_gametype.integer == GT_CTF || g_gametype.integer == GT_CTY 
                        || g_gametype.integer == GT_SABER_RUN) {
...
============
*/
gentity_t *SelectCTFSpawnPoint ( gentity_t *ent, vec3_t origin, vec3_t angles ) {
	gentity_t	*spot;

	spot = SelectRandomTeamSpawnPoint ( ent->client->pers.teamState.state, ent->client->sess.sessionTeam, ent->client->siegeClass );
...
gentity_t *SelectSiegeSpawnPoint ( gentity_t *ent, vec3_t origin, vec3_t angles ) {
	gentity_t	*spot;
