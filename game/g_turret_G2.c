...
#define SPF_TURRETG2_TURBO		8 //This is wrong but it is this way in siege
//#define SPF_TURRETG2_TURBO		4
//#define SPF_TURRETG2_CANRESPAWN	        8
#define SPF_TURRETG2_LEAD_ENEMY	        16
#define SPF_SHOWONRADAR			32
#define SPF_TARGET_FIGHTERS             64 //Won't target ppl
#define SPF_TARGET_GROUND_VEHICLES      128//Won't target ppl
#define SPF_TARGET_ARMED                256//Won't target unarmed ppl
//RoboPhred
#define SPF_TARGET_PLAYERKEYS			(512 | 1024)
...
//Lugormod
qboolean isArmed (gentity_t *ent) {
	if (!ent->client) {
		return qfalse;
	}
	if ((ent->client->ps.weapon == WP_SABER && ent->client->ps.saberHolstered == 2)
		|| ent->client->ps.weapon == WP_BRYAR_OLD
...
qboolean TurretG2_CanTarget(gentity_t *self, gentity_t *target) {

	
	if(self->alliedTeam != TEAM_FREE && self->alliedTeam == target->alliedTeam)
		return qfalse;

	if(PlayerUseableCheck(self, target) == (self->spawnflags & SPF_TARGET_PLAYERKEYS))
		return qfalse;

	if(!(self->spawnflags & (SPF_TARGET_FIGHTERS | SPF_TARGET_GROUND_VEHICLES | SPF_TARGET_ARMED)))
		return qtrue;

	qboolean isEnemy = qfalse;
	if(target->m_pVehicle) {
		if(self->spawnflags & SPF_TARGET_FIGHTERS && target->m_pVehicle->m_pVehicleInfo->type == VH_FIGHTER)
			return qtrue;
		else if(self->spawnflags & SPF_TARGET_GROUND_VEHICLES && target->m_pVehicle->m_pVehicleInfo->type != VH_FIGHTER)
			return qtrue;
	}
	if(self->spawnflags & SPF_TARGET_ARMED && isArmed(target))
		return qtrue;
...
//------------------------------------------------------------------------------------------------------------
{
	...
	if ( !self->enemy && TurretG2_CanTarget(self, attacker))
...
//------------------------------------------------------------------------------------------------------------
{
	...
	else
	{
		...
		if ((self->spawnflags&SPF_TURRETG2_TURBO) &&
			(self->spawnflags&SPF_TURRETG2_CANRESPAWN)) { 
				//Lugormod want to respawn
				VectorCopy( self->r.currentAngles, self->s.apos.trBase );
				VectorClear( self->s.apos.trDelta );
...
//----------------------------------------------------------------
{
	...
	else
	{
		...
		if (g_gametype.integer == GT_BATTLE_GROUND &&
			ent->parent && ent->parent->alliedTeam) { //Lugormod
				bolt->alliedTeam = ent->parent->alliedTeam;
		} else {
...
void turretG2_respawn( gentity_t *self )
{
	...
	if (( self->s.eFlags & EF_SHADER_ANIM ) && ( self->spawnflags & 1 ))//Lugormod can be activated when dead
	{
		self->s.frame = 1; // black
	}
	else {
...
//-----------------------------------------------------
{
	...
	else
	{
		...
		//WFMTree: who gives a crap about walls, do it anyway
		//WFMTree: thing has to be on, and not a turbo
		if(!(self->spawnflags & SPF_TURRETG2_TURBO) && self->s.frame == 0)
			//WFMTree: i need to check this later when my brain is running at full again,
			//want to make this only rotate +/- 45 from where it was last aiming.
	...
	{
		self->s.loopSound = 0;
		VectorSet(self->s.apos.trDelta, 0,0,0);
	}
}
...
//-----------------------------------------------------
{
	...
	if (g_gametype.integer == GT_BATTLE_GROUND &&
		(!self->parent || !self->parent->alliedTeam)) {
	...
	count = G_RadiusList( org2, self->radius, self, qtrue, entity_list, qfalse );
	...
	for ( i = 0; i < count; i++ )
	{
		...
		{
			/*
			RoboPhred: Old code for turrets attacking sp ships (which were misc_model_breakable-s)
			Causes oddities in mp, so disabling...
			A critical flaw here is that it targets based on its own targetname, not target, else I could keep this
			by ONLY attacking when we are specifically told to.
			*/
			// only attack clients
		...
		if (target->client && (TurretG2_CanTarget(self, target) == qfalse || target->client->sess.sessionTeam == TEAM_SPECTATOR)) {
			continue;
		}

		if ( g_gametype.integer == GT_BATTLE_GROUND) //Lugormod
		{
			if ( target->m_pVehicle ) {
				if ( !target->m_pVehicle->m_pPilot ||
					((gentity_t*)target->m_pVehicle->m_pPilot)->client->sess.sessionTeam == self->parent->alliedTeam) {
						continue;
				}
			...
			{
				if ( target->client->sess.sessionTeam == self->parent->alliedTeam )
				{ 
					// A bot/client/NPC we don't want to shoot
					continue;
				}
			}
			else if ( target->teamnodmg == self->parent->alliedTeam )
			{ 
				// An ent we don't want to shoot
				continue;
			}
		}
		else if ( self->alliedTeam ){
			if ( target->m_pVehicle ) { //Lugormod if in vehicle
				if ( !target->m_pVehicle->m_pPilot ||
					((gentity_t*)target->m_pVehicle->m_pPilot)->client->sess.sessionTeam == self->alliedTeam) {
						continue;
				}
	...
	if ( self->enemy != NULL )
	{
		...
		{//FIXME: remain single-minded or look for a new enemy every now and then?
			...
			if ( enemyDist < self->radius * self->radius )
			{
				...
				//end Lugormod
				if ( trap_InPVS( org2, //self->r.currentOrigin
...
//-----------------------------------------------------------------------------
{
	...
	if ((( self->s.eFlags & EF_SHADER_ANIM ) && ( self->spawnflags & 1 ))
		|| self->health < 1)//Lugormod can be activated when dead // Start_Off
	{
		self->s.frame = 1; // black
	}
	else
...
//-----------------------------------------------------
{
	...
	//RoboPhred: why the heck did you do this lugor?
	//oh, stupid ravensoft has the cvars flipped...
	//so what format do we use?  mp or sp?
	//most lugormod servers use sp maps... but then again, very few sp maps
	//have turrets...
	//meh, cvarifying it: lmd_spturrets
	if (g_gametype.integer != GT_SIEGE && lmd_spturrets.integer) { //Lugormod fix 
		int tmpflags = 0;
		if (base->spawnflags&SPF_TURRETG2_TURBO) {
			tmpflags |= SPF_TURRETG2_CANRESPAWN;
		}
		if (base->spawnflags&SPF_TURRETG2_CANRESPAWN) {
			tmpflags |= SPF_TURRETG2_TURBO;
		}
		base->spawnflags &= ~(SPF_TURRETG2_TURBO|SPF_TURRETG2_CANRESPAWN);
		base->spawnflags |= tmpflags;
	}
...
#undef TURRET_MODEL_CANNON
#undef TURRET_MODEL_DAMAGE
#undef TURRET_MODEL_LASER
}
