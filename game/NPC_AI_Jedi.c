...
void Boba_FireFlameThrower( gentity_t *self )
{
	...
	{
		//RoboPhred
		G_Damage( traceEnt, self, self, dir, tr.endpos, damage, DAMAGE_NO_KNOCKBACK|/*DAMAGE_NO_HIT_LOC|*/DAMAGE_IGNORE_TEAM, MOD_LAVA );
...
void Jedi_Cloak( gentity_t *self )
{
	if ( self )
	{
		...
		if ( self->client )
		{
			...
			if(self->client->ps.m_iVehicleNum && lmd_vehcloaking.integer != 0) {
				targ = &g_entities[self->client->ps.m_iVehicleNum];
				//Only the pilot can control our cloaking
				if(self == (gentity_t *)targ->m_pVehicle->m_pPilot) {
					if(!targ->client->ps.powerups[PW_CLOAKED]) {
						targ->client->ps.powerups[PW_CLOAKED] = INT_MAX;//Q3_INFINITE;
						doSound = qtrue;
					}
				}
			...
			{//cloak
				self->client->ps.powerups[PW_CLOAKED] = INT_MAX;//Q3_INFINITE;
				doSound = qtrue;
			}
			if(doSound) {
