...
void NPC_SpawnEffect (gentity_t *ent)
{
	...
	//G_PlayEffectID(G_EffectIndex("effects/misc/possession"),orig , upAng);
	ent->client->ps.powerups[PW_CLOAKED] = 500;
	//G_Sound(ent,CHAN_AUTO,G_SoundIndex("sound/effects/vacuum"));
}
...
void NPC_Begin (gentity_t *ent)
{
	...
	//RoboPhred: These two are already set...
	/*
	ent->inuse = qtrue;
	ent->classname = "NPC";
	*/
	...
	*/
	...
	//RoboPhred: Redundant, was already set.
	//MCG - Begin: NPC hacks
...
gentity_t *NPC_Spawn_Do( gentity_t *ent )
{
	...
	*/
	...
	newent->classname = "NPC";
	newent->NPC_type = G_NewString(ent->NPC_type);
...
RoboPhred
NPC_target3 - target to use on npc death, player as activator
...
gentity_t *NPC_SpawnType( gentity_t *ent, char *npc_type, char *targetname, qboolean isVehicle ) 
{
	...
	/*
	if (targetname 
	&& (Q_stricmpn("spawner",targetname,7) == 0)
	&& atoi(targetname + 7) > 0){
	//NPCspawner->use = NPC_Spawn;

	//NPCspawner->think = NPC_Spawn_Go;
	//NPCspawner->r.svFlags |= SVF_NPC_PRECACHE;
	...
	NPCspawner->count = atoi (targetname + 7);
	...
	NPCspawner->NPC_targetname = G_NewString(va ("%s%i",
	npc_type,
	NPCspawner->s.number));
	//NPCspawner->targetname = G_NewString("TjoSpawner");
	trap_LinkEntity(NPCspawner);
	return NPCspawner;
	}
	else
...
/*
void PutSpawnerInWorld (gentity_t *NPCspawner, const char *npc_type, qboolean isVehicle)
{
NPCspawner->NPC_type = G_NewString( npc_type );
