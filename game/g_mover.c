...
qboolean	G_TryPushingEntity( gentity_t *check, gentity_t *pusher, vec3_t move, vec3_t amove ) {
	...
	*/
	...
	if (block && block-g_entities < MAX_CLIENTS) { //Lugormod ppl push ppl
		if (G_TryPushingEntity(block, check, move, amove)) {
			block = NULL;
		}
	}
...
void G_MoverTeam( gentity_t *ent ) {
	...
	for (part = ent ; part ; part=part->teamchain) {
		...
		//if (part->model2) {//Lugormod
		//        VectorAdd (angles, part->s.angles2,amove);
		//}
...
{
	//RoboPhred: ravensoft made this completely wrong, and the places they called it from never actually used it.
...
void SetMoverState( gentity_t *ent, moverState_t moverState, int time ) {
	...
	case MOVER_POS1:
		...
		//RoboPhred
		VectorCopy( ent->pos3, ent->s.apos.trBase );
	...
	case MOVER_POS2:
		...
		//RoboPhred
		VectorCopy( ent->pos4, ent->s.apos.trBase );
		ent->s.apos.trType = TR_STATIONARY;
	...
	case MOVER_1TO2:
		...
		//RoboPhred
		VectorCopy( ent->pos3, ent->s.apos.trBase );
		VectorSubtract( ent->pos4, ent->pos3, delta );
	...
	case MOVER_2TO1:
		...
		//RoboPhred
		VectorCopy( ent->pos4, ent->s.apos.trBase );
		VectorSubtract( ent->pos3, ent->pos4, delta );
		f = 1000.0 / ent->s.pos.trDuration;
		VectorScale( delta, f, ent->s.apos.trDelta );
		if ( ent->alt_fire )
		{
...
void Use_BinaryMover_Go( gentity_t *ent ) 
{
	...
	if ( ent->moverState == MOVER_2TO1 ) 
	{
		...
		//RoboPhred: rotating doors
		if ( ent->s.apos.trType == TR_NONLINEAR_STOP )
		{
			vec3_t curDelta;
			float fPartial;
			total = ent->s.apos.trDuration-50;
			VectorSubtract( ent->r.currentOrigin, ent->s.angles, curDelta );
			fPartial = VectorLength( curDelta )/VectorLength( ent->s.apos.trDelta );
			VectorScale( ent->s.apos.trDelta, fPartial, curDelta );
			fPartial /= ent->s.apos.trDuration;
			fPartial /= 0.001f;
			fPartial = acos( fPartial );
			fPartial = RAD2DEG( fPartial );
			fPartial = (90.0f - fPartial)/90.0f*ent->s.apos.trDuration;
			partial = total - floor( fPartial );
		}
		else
		{
		...
		ent->s.apos.trTime = level.time - ( total - partial );//ent->s.time;
	...
	if ( ent->moverState == MOVER_1TO2 ) 
	{
		...
		if ( ent->s.apos.trType == TR_NONLINEAR_STOP )
		{
			...
			float fPartial;
			total = ent->s.apos.trDuration-50;
			VectorSubtract( ent->r.currentAngles, ent->pos4, curDelta );
			fPartial = VectorLength( curDelta )/VectorLength( ent->s.apos.trDelta );
			VectorScale( ent->s.apos.trDelta, fPartial, curDelta );
			fPartial /= ent->s.apos.trDuration;
			fPartial /= 0.001f;
...
//extern vmCvar_t g_dontLoadNPC; //Lugormod now in g_local
...
void UnLockDoors(gentity_t *const ent)
{
	...
	{	// want to allow locked toggle doors, so keep the targetname
		if( g_gametype.integer != GT_SIEGE && !(slave->spawnflags & MOVER_TOGGLE) && !g_dontLoadNPC.integer)// Lugormod
		{
...
void LockDoors(gentity_t *const ent)
{
	...
	//if (g_dontLoadNPC.integer) { //Lugormod
	//        return;
	//}
...
void Use_BinaryMover( gentity_t *ent, gentity_t *other, gentity_t *activator ) 
{
	...
	//RoboPhred
	if(Q_stricmp(ent->classname, "lmd_door") == 0 && !PlayerUseableCheck(ent, activator))
		return;
	...
	//Lugormod make security panels toggle locked
	if (g_gametype.integer != GT_SIEGE && g_dontLoadNPC.integer && Q_stricmp("misc_security_panel", other->classname) == 0) {
		LockDoors(ent);
		return;
	}
...
void InitMoverTrData( gentity_t *ent )
{
	...
	//RoboPhred
	ent->s.apos.trType = TR_STATIONARY;
	//Already done as basic entity stuff.
	//VectorCopy( ent->s.angles, ent->s.apos.trBase );

	// calculate time to reach second position from speed
	VectorSubtract( ent->pos4, ent->pos3, move );
	distance = VectorLength( move );
...
{
	//Ufo: clean up a dropable item from the path
	if (other->physicsObject)
		other->physicsObject = qfalse;
...
gentity_t* find_target(gentity_t *ent){
	int i;
	gentity_t *cEnt;

	if (!ent || !ent->inuse || !ent->targetname) {
		return NULL;
	}
...
qboolean G_EntIsDoor( int entityNum )
{
	...
	//RoboPhred
	if (ent && (!Q_stricmp( "func_door", ent->classname) || !Q_stricmp( "lmd_door", ent->classname)))
...
void Touch_DoorTrigger( gentity_t *ent, gentity_t *other, trace_t *trace ) 
{
	...
	//Lugormod
	/*
	if (ent->parent->targetname && g_dontLoadNPC.integer
	&& (g_gametype.integer == GT_FFA || 
	g_gametype.integer == GT_TEAM)
	&& ent->genericValue12) {

	gentity_t *cEnt;
	cEnt = find_target(ent->parent);
	if (cEnt) {
	if (Q_stricmp(cEnt->classname, "trigger_multiple")) {
	//Don't want this.
	//gentity_t *last;
	//last = cEnt;
	//while (cEnt = find_target(last)) {
	//        G_FreeEntity(last);
	//        last = cEnt;
	// }
	//G_FreeEntity(cEnt);
	} /*else if (cEnt->targetname) {
	gentity_t *bEnt = find_target(cEnt);
	if (bEnt) {
	if (Q_stricmp(bEnt->classname, 
	"trigger_once") == 0) {
	G_FreeEntity(cEnt);
	G_FreeEntity(bEnt);
	G_Free(ent->parent->targetname);
	ent->parent->targetname = NULL;
	} else {
	//All targeters present
	...
	//Second targeter is missing
	//Trigger should stay, 
	//but dont do this check again
	G_FreeEntity(cEnt);
	G_Free(ent->parent->targetname);
	ent->parent->targetname = NULL;
	}
	...
	ent->genericValue12 = 0;
	} //else {
	//First targeter is missing
	//Don't do this check again
	//G_Free(ent->parent->targetname);
	//ent->parent->targetname = NULL;
	//}
	} //else if (ent->parent->script_targetname && g_dontLoadNPC.integer) {
	//}
...
//RoboPhred
void FreeDoorTrigger(gentity_t *door) {
	gentity_t *trig = NULL;
	while((trig = G_Find(trig, FOFS(classname), "trigger_door"))) {
		if(trig->parent == door)
			G_FreeEntity(trig);
	}
}
void Think_SpawnNewDoorTrigger( gentity_t *ent ) 
{
	...
	//RoboPhred
	ent->freeEntity = FreeDoorTrigger;

	trap_LinkEntity (other);
	other->classname = "trigger_door";
	// remember the thinnest axis
	other->count = best;
	//other->genericValue12 = 1;
...
void SP_func_door (gentity_t *ent) 
{
	...
	if (g_gametype.integer != GT_SIEGE &&
		g_dontLoadNPC.integer) {
			//Lugormod try again
			if ((ent->spawnflags & MOVER_TOGGLE) &&
				!(ent->spawnflags & MOVER_PLAYER_USE)&&
				(ent->spawnflags & MOVER_START_ON)) {
					ent->spawnflags &= ~(MOVER_TOGGLE|MOVER_START_ON);
					ent->targetname = NULL;
			}        
	...
	*/
	...
	//G_SpawnVector("modelAngles", "0 0 0",
	//              modelangles);
	//        G_SpawnFloat("angle",va("%f", ent->s.angles[1]), 
	//                     &ent->s.angles[1]);
	//}
	//end Lugormod
	vec3_t	abs_movedir;
	...
	//RoboPhred
	qboolean NeedsDoorTrig = qfalse;
	if(ent->Lmd.spawnData && ent->team)
	...
	//RoboPhred
	if(lmd_enforceentwait.integer && ent->wait < lmd_enforceentwait.integer)
		ent->wait = lmd_enforceentwait.integer; 
	...
	//RoboPhred
	if(g_gametype.integer == GT_FFA && ent->alliedTeam){
		ent->alliedTeam = 0;
		ent->spawnflags &= ~MOVER_LOCKED;
		//locked doors can be hacked by others to become unlocked,
		//and since this wont occure now...
	...
	//trap_LinkEntity(ent);
	//RoboPhred
	if((!ent->targetname || !ent->targetname[0]) && !(ent->spawnflags & 8))
		NeedsDoorTrig = qtrue;
	trap_SetBrushModel( ent, ent->model );
	//VectorCopy(ent->s.angles2, ent->s.apos.trBase); //Lugormod
	...
	if ( ent->spawnflags & MOVER_START_ON ) 
	{
		vec3_t	temp;
	...
	if ( !(ent->flags&FL_TEAMSLAVE) ) 
	{
		...
		//RoboPhred
		//if ( !(ent->spawnflags&MOVER_LOCKED) && (/*(*/ent->targetname /*&& g_gametype.integer != GT_FFA && g_gametype.integer != GT_TEAM) */|| health || ent->spawnflags & MOVER_PLAYER_USE || ent->spawnflags & MOVER_FORCE_ACTIVATE) )
		if(!(ent->spawnflags&MOVER_LOCKED) && (!NeedsDoorTrig || health || ent->spawnflags & MOVER_PLAYER_USE || ent->spawnflags & MOVER_FORCE_ACTIVATE)){
...
void fixdoor(gentity_t *self){
	...
	while ( (t = G_Find (t, FOFS(targetname), self->target)) != NULL ) {
		if ( t == self ) {
			...
			if (Q_stricmp("func_door", t->classname)) {
				continue;
			}
			//should go through all trigger_door
			//and check if there already is a
			//trigger for this door.
			t->think = Think_SpawnNewDoorTrigger;
			t->nextthink = level.time + 100;
			t->wait = 2000;
		}
	}
	//G_Free(self->target);
...
void SP_target_fixdoor (gentity_t *ent){
	//RoboPhred
	ent->think = fixdoor;
	ent->nextthink = level.time + FRAMETIME;
	//ent->use = fixdoor;
...
*/
//RoboPhred
const entityInfoData_t func_plat_spawnflags[] = {
	{"64", "Activate when used by the player."},
	{"128", "Start deactivated."},
	{NULL, NULL}
};
const entityInfoData_t func_plat_keys[] = {
...
void Reached_Train( gentity_t *ent ) {
	...
	//RoboPhred: spawnflag 1 on path_corner has the train stop here.  Using it will start it going again.
...
void SP_func_static( gentity_t *ent ) 
{
	...
	G_SetOrigin( ent, ent->s.origin );
	//G_SetAngles( ent, ent->s.angles ); //Lugormod
	...
	//G_SpawnInt( "hyperspace", "0", &test ); //Lugormod Huh ??
	/*
	if ( test )
	{
	ent->r.svFlags |= SVF_BROADCAST; // I need to rotate something that is huge and it's touching too many area portals...
...
void SP_func_rotating (gentity_t *ent) {
	...
	//RoboPhred: Damn you raensoft for forgetting this
...
{//Toggle on and off
	...
	else
	{
		...
		self->think = 0;
		self->nextthink = -1;
		//Lugormod we shouldn't have any further use for this
		//G_FreeEntity(self);
	}
...
&& ent->targetname) {
gentity_t *last, *cEnt;
if (!(cEnt = find_target(ent)) 
|| (cEnt 
&& Q_stricmp(cEnt->classname, "trigger_once") == 0)) {
if (ent->use) {
...
void SP_func_usable( gentity_t *self ) 
{
	...
	*/
	...
	//Lugormod
	//if (!(self->spawnflags & MOVER_PLAYER_USE)) {
	//        self->touch = Touch_func_usable;
...
void rail_mover_think (gentity_t *ent) {
	if (ent->flags & FL_INACTIVE || !ent->parent || !ent->parent->use ) {
		ent->nextthink = level.time;
		return;
	}
	if (ent->parent->genericValue6 > level.time) {
		ent->r.svFlags |= SVF_NOCLIENT;
		ent->s.eFlags |= EF_NODRAW;
		ent->r.contents = 0;
		ent->nextthink = ent->parent->genericValue6;
		return;
	}

	if ((ent->parent->pos1[0] > 0.1f && ent->r.absmax[0] > ent->parent->r.absmax[0])
		|| (ent->parent->pos1[0] < -0.1f && ent->r.absmin[0] < ent->parent->r.absmin[0])
		|| (ent->parent->pos1[1] > 0.1f && ent->r.absmax[1] > ent->parent->r.absmax[1])
		|| (ent->parent->pos1[1] < -0.1f && ent->r.absmin[1] < ent->parent->r.absmin[1])
		|| (ent->parent->pos1[2] > 0.1f && ent->r.absmax[2] > ent->parent->r.absmax[2])
		|| (ent->parent->pos1[2] < -0.1f && ent->r.absmin[2] < ent->parent->r.absmin[2])){

			ent->s.pos.trType = TR_STATIONARY;
			VectorSet(ent->s.pos.trDelta, 0, 0,0);

			if (ent->parent->flags & FL_INACTIVE) {
				ent->r.svFlags |= SVF_NOCLIENT;
				ent->s.eFlags |= EF_NODRAW;
			} else {
				ent->s.eFlags &= ~EF_NODRAW;
				ent->r.svFlags &= ~SVF_NOCLIENT;
				ent->parent->use(ent->parent, ent, ent);
			}
			//ent->s.pos.trTime = level.time;
			ent->nextthink = level.time + 100;
			ent->r.contents = 0;
			ent->genericValue9 = level.time - ent->genericValue8 - 50;
			trap_LinkEntity(ent);
	} else {
		if (ent->s.pos.trType != TR_LINEAR ) {
			vec3_t dir;
			VectorCopy(ent->parent->pos1, dir);
			VectorNormalize(dir);

			VectorScale(dir, ent->parent->speed * 500, ent->s.pos.trDelta);
			ent->s.pos.trTime = level.time;
			...
			trap_LinkEntity(ent);
			ent->nextthink = level.time + ent->genericValue9;
			ent->genericValue8 = level.time;
		}
		ent->nextthink = level.time;
	}
}

void rail_mover_init (gentity_t *ent) {        
	gentity_t *track;        
	track = G_PickTarget(ent->target);
	ent->parent = track;
	...
	ent->s.pos.trTime = level.time;
	ent->s.eType = ET_MOVER;
	ent->think = rail_mover_think;
	ent->nextthink = level.time + 100;
	trap_LinkEntity( ent );
	VectorSubtract(ent->r.absmax, ent->r.absmin, ent->pos2);
}
...
void SP_rail_mover ( gentity_t *ent ) {
	...
	ent->nextTrain = NULL;
	ent->prevTrain = NULL;
	ent->genericValue9 = 500;
	trap_LinkEntity(ent);

}

qboolean boxEmpty(gentity_t *ent, const vec3_t mins, const vec3_t maxs) {
	gentity_t *check;
	vec3_t mySize, size;
	VectorSubtract(maxs,mins,mySize);
	int i,n;
	qboolean ok;

	for (n = MAX_CLIENTS;n < level.num_entities;n++) {
		check = &g_entities[n];
		if (!check || !check->inuse || check->s.eType != ET_MOVER || !check->parent)
			continue;
		if (check->parent != ent && (!check->parent->parent || check->parent->parent != ent))
			continue;

		VectorSubtract(check->r.absmax, check->r.absmin, size);
		VectorAdd(size, mySize, size);
		ok = qfalse;
		for (i = 0; i < 3;i++) {
			if ((check->r.absmin[i] < maxs[i] &&
				maxs[i] - check->r.absmin[i] > size[i] 
			+ ent->radius) ||
				(mins[i] < check->r.absmax[i] &&
				check->r.absmax[i] - mins[i] > size[i]
			+ ent->radius)) {
				ok = qtrue;
				break;
			}
...
void rail_lane_think (gentity_t *ent){
	ent->nextthink = level.time + 100;
	if (!ent->nextTrain || !ent->parent || !ent->parent->use)
		return;
	gentity_t *mover = ent->nextTrain;
	ent->nextTrain = mover->nextTrain;
	mover->nextTrain = NULL;
	ent->parent->use(ent->parent, mover, ent);
}

void rail_track_think (gentity_t *ent){
	if (ent->nextTrain || (ent->prevTrain && ent->prevTrain->genericValue7 == 0)) {
			gentity_t *mover;
			if (ent->prevTrain && ent->prevTrain->genericValue7 == 0) {
					mover = ent->prevTrain;
			} else {
				mover = ent->nextTrain;
				if (ent->count && mover->parent == ent) {
					gentity_t *t = ent;
					while (t->prevTrain) {
						t = t->prevTrain;
					}
					...
					ent->nextTrain = mover->nextTrain;
					mover->nextTrain = NULL;
					mover->prevTrain = NULL;
					mover->genericValue7 = ent->count;
					ent->nextthink = level.time + 100;
					return;
				}
			}
			...
			vec3_t mins, maxs;
			VectorSubtract(mover->r.absmin, ent->pos1, mins);
			VectorSubtract(mover->r.absmax, ent->pos1, maxs);
			if (boxEmpty(ent, mins, maxs)) {
				VectorSubtract(mover->r.currentOrigin, ent->pos1, mover->r.currentOrigin);
				VectorCopy(mover->r.currentOrigin, mover->s.pos.trBase);
				if (ent->count && mover->parent == ent) {
					ent->prevTrain = mover->prevTrain;
				} else {
					ent->nextTrain = mover->nextTrain;
					if (ent->prevTrain) {
						ent->prevTrain->genericValue7--;
					}
				}
				mover->prevTrain = NULL;
				mover->nextTrain = NULL;
				mover->flags &= ~FL_INACTIVE;
...
void rail_lane_init (gentity_t *ent){
	vec3_t dir, diff;

	ent->parent = G_PickTarget(ent->target);
	if (!ent->parent) {
		return;
	}

	VectorCopy(ent->parent->s.angles, ent->s.angles);
	AngleVectors(ent->s.angles, dir, NULL, NULL);
	dir[0] = Q_round(dir[0]);
	dir[1] = Q_round(dir[1]);
	dir[2] = Q_round(dir[2]);
	VectorNormalize(dir);
	VectorSubtract(ent->r.absmax, ent->r.absmin, diff);
	diff[0] *= dir[0];
	diff[1] *= dir[1];
	diff[2] *= dir[2];
	VectorCopy(diff, ent->pos1);
	ent->think = rail_lane_think;
	ent->nextTrain = NULL;
	if (ent->delay) {
		ent->genericValue6 = level.time + ent->delay;
	} else {
		ent->genericValue6 = 0;
	}
	ent->nextthink = level.time + 100;
...
void rail_track_init (gentity_t *ent)
{
	vec3_t dir, diff;
	AngleVectors(ent->s.angles, dir, NULL, NULL);
	dir[0] = Q_round(dir[0]);
...
void rail_track_use (gentity_t *self, gentity_t *other, gentity_t *activator) {

	if (!other || !other->inuse || other->s.eType != ET_MOVER) {
		return;
	}
	gentity_t *t;
	t = self;
...
void rail_lane_use (gentity_t *self, gentity_t *other, gentity_t *activator) {
	if (!other || !other->inuse || other->s.eType != ET_MOVER) {
		return;
	}
	gentity_t *t;
...
void SP_rail_lane ( gentity_t *ent ){
	...
	ent->s.eType = ET_INVISIBLE;
	trap_SetBrushModel( ent, ent->model );        
	ent->use = rail_lane_use;
	if (ent->spawnflags & 1) {
		ent->flags |= FL_INACTIVE;
	}
	ent->nextthink = level.time + 100;
	ent->think = rail_lane_init;
	ent->nextTrain = NULL;
	ent->genericValue5 = 0;
	trap_LinkEntity(ent);
}
