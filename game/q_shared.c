...
char *COM_ParseLine( const char **data_p){
	...
	while ( *data ){
		...
		if (c == '\"') {
			...
			// Parsing entire line, need to also parse quotes.
			//data++;
			//continue;
		}
...
int Q_stricmpword (const char *s1, const char *s2) {
	...
	do {
		/*
		while (!Q_isalpha(*s1) && *s1){
		s1++;
		}
		*/

		while (!Q_isalpha(*s2) && *s2){
			s2++;
		}
...
int Q_stricmpname (const char *s1, const char *s2) {
	...
	do {
		while (!Q_isalpha(*s1) && *s1 && (s1 - st1) < MAX_NAME_LENGTH){
			s1++;
		}
		while (!Q_isalpha(*s2) && *s2 && (s2 - st2) < MAX_NAME_LENGTH){
			s2++;
		}
		...
		if ((s1 - st1) > MAX_NAME_LENGTH ||
			(s2 - st2) > MAX_NAME_LENGTH) {
				return 0;		// strings are equal until end point
		}
...
static char numbertoletter[10] = {'o','1','z','e','4','s','g','7','8','g'};
...
char *Q_SkipTag(char *string) {
	...
	if (d - s < 2 || d - s > 6) { //if it's a tag, it's probably between 1 and 5 chars.
		return string;
	}

	//Start lookin for symmetry
	while ( s >= string && (*d - *s) < 3 && (*d - *s) > -3 && *d) {
		d++;
		s--;
	}
...
char *Q_StripTags( char *string ) {
	...
	while ((c = *s) != 0 ) {
		if ( (c >= 'a' && c <= 'z') || (c >= 'A' && c <= 'Z') || c == ' ') {
			if (c <= 'Z' && c != ' ') {
				c += ('a'-'A');
			} //lets make it lowercase

			*d++ = c;
		} else {
			t = Q_SkipTag(s);
			if (d - string + strlen(t + 1) > 2) {
				s = t;
			}
...
qboolean Q_wordInLine (const char *word,/*int wordl,*/const char *line) {
	...
	do {
		...
		if (line[i - 1] == ' ' 
			...
			//|| line[i - 1] == '\r' 
			|| line [i - 1] == '\0') {
				if (/*(i - j - 1 == wordl) &&*/Q_stricmpword(word, &(line[j])) == 0) {
					return qtrue;
...
qboolean
Q_wordsInLine(const char *words,const char *line, char **next) 
{
	...
	do {
		...
		if (//ans && 
			...
			|| words[i-1] == '\r' 
			|| words[i-1] == '\n')) {
				if (!Q_wordInLine(&words[j],/* i - j - 1,*/ line)) {
					*next = skipline(words);
					return qfalse;
					//ans = qfalse;
				}
