...
void NPC_BSStandGuard (void)
{
	...
	{//Possible to pick one up by being shot
		if( random() < 0.5 )
		{
			...
			{
				gentity_t *newenemy = NPC_PickEnemy(NPC, NPC->client->enemyTeam, (qboolean)(NPC->cantHitEnemyCounter < 10), (qboolean)(NPC->client->enemyTeam == NPCTEAM_PLAYER), qtrue);
...
void NPC_BSHuntAndKill( void )
{
	...
	//NPC_CheckEnemy( qboolean(NPCInfo->tempBehavior != BS_HUNT_AND_KILL), qfalse, qtrue );//don't find new enemy if this is tempbehav
...
//	qboolean	shoot = qfalse;
	...
	NPC_CheckEnemy( (qboolean)(NPCInfo->scriptFlags&SCF_LOOK_FOR_ENEMIES), qfalse, qtrue );
	if ( !NPC->enemy )
