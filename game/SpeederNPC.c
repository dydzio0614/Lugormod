...
//Lugormod size scaling
	...
	if ( pVeh->m_ulFlags & VEH_FLYING ) 
	{
		speedInc = pVeh->m_pVehicleInfo->acceleration * pVeh->m_fTimeModifier * 0.4f * scaleh;
	}
#ifdef _JK2MP
	...
	{
		speedInc = pVeh->m_pVehicleInfo->acceleration * pVeh->m_fTimeModifier * scaleh;
	}
	speedIdleDec = pVeh->m_pVehicleInfo->decelIdle * pVeh->m_fTimeModifier * scaleh;
		...
		)
	{
			  ...
			  (pVeh->m_pPilot->playerState->weapon == WP_SABER && BG_SabersOff( pVeh->m_pPilot->playerState ) ))) )
		{
			if ((curTime - pVeh->m_iTurboTime)>pVeh->m_pVehicleInfo->turboRecharge)
			{
				...
				parentPS->speed = pVeh->m_pVehicleInfo->turboSpeed * scaleh;	// Instantly Jump To Turbo Speed
			}
		}
	}
	...
	{
		speedMax = pVeh->m_pVehicleInfo->turboSpeed * scaleh;
		if (parentPS)
		{
	...
	{
		speedMax = pVeh->m_pVehicleInfo->speedMax * scaleh;
		if (parentPS)
		{
			parentPS->eFlags &= ~EF_JETPACK_ACTIVE;
		}
	...
	speedIdle = pVeh->m_pVehicleInfo->speedIdle * scaleh;
	speedIdleAccel = pVeh->m_pVehicleInfo->accelIdle * pVeh->m_fTimeModifier * scaleh;
	speedMin = pVeh->m_pVehicleInfo->speedMin * scaleh;
...
void AnimateRiders( Vehicle_t *pVeh )
{
	...
	{
		bool		HasWeapon	= (bool)((pilotPS->weapon != WP_NONE) && (pilotPS->weapon != WP_MELEE));
		bool		Attacking	= (bool)(HasWeapon && !!(pVeh->m_ucmd.buttons&BUTTON_ATTACK));
...
#endif
		bool		Right		= (bool)(pVeh->m_ucmd.rightmove>0);
		bool		Left		= (bool)(pVeh->m_ucmd.rightmove<0);
		bool		Turbo		= (bool)(curTime<pVeh->m_iTurboTime);
