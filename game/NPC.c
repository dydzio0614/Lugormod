...
extern void NPC_BSHowler_Default( void );
extern void NPC_BSSand_creature_Default( void );
...
void NPC_ExecuteBState ( gentity_t *self)//, int msec ) 
{
	...
	gentity_t backupE;
	gclient_t backupC;
	memcpy(&backupE, self, sizeof(gentity_t));
	memcpy(&backupC, client, sizeof(gclient_t));

	int restore = 0;
	if (restore) {
		memcpy(self, &backupE, sizeof(gentity_t));
		memcpy(self->client, &backupC, sizeof(gclient_t));
		client = self->client;
	}
...
{	// FIXME : once torsoAnim and legsAnim are in the same structure for NCP and Players
	...
	G_SetAnim(ent, setAnimParts, anim, setAnimFlags, 0);
/*
	if(ent->client)
	{//Players, NPCs
