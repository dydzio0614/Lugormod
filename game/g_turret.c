...
//-----------------------------------------------------
{
	...
	// RoboPhred: Do not make a sound for tiny movements
	if ( diffYaw > 1.25 || diffPitch > 1.25)
	{
...
//-----------------------------------------------------------------------------
{
	...
	//RoboPhred: if we are toggled on, start thinking
	if(!(self->spawnflags & 1)) {
		self->flags &= ~FL_NOTARGET;
		self->nextthink = level.time + FRAMETIME;
	}
...
void turret_component_free(gentity_t *base) {
	// RoboPhred: If we got freed, clear the other part
	if (base->target_ent) {
		gentity_t *other = base->target_ent;

		// Clear our var so we dont loop.
		base->target_ent = NULL;
...
//-----------------------------------------------------
{
	...
	*/
	...
	// RoboPhred
	base->freeEntity = turret_component_free;
}
...
qboolean turret_base_spawn_top( gentity_t *base )
{
	...
	// RoboPhred
	top->freeEntity = turret_component_free;
