...
//Lugormod
/*RoboPhred: made a much better way
gentity_t* create_body_double (gentity_t *ent)
{
	gentity_t		*body;
	int			islight = 0;
	...
	body->r.contents = CONTENTS_SOLID;//CONTENTS_CORPSE;
	body->r.ownerNum = ent->s.number;

	//body->nextthink = level.time + 30000;
	//body->think = body_double_think;

	//body->die = body_double_die;
...
void camera_disconnect(gentity_t *camera, gentity_t *player, gentity_t *body, vec3_t origin){
	...
	if(player && player->client && player->client->pers.connected == CON_CONNECTED){
		...
		//I don't think we have wo worry about the angles2 value being invalid like the origin.
		VectorCopy(camera->s.angles2, player->client->ps.viewangles);
...
void SP_misc_security_panel (gentity_t *ent) {
	...
	ent->s.modelindex = G_ModelIndex("models/map_objects/kejim/sec_panel.md3");
	VectorCopy(ent->s.angles, ent->s.apos.trBase);
	ent->use = misc_security_panel_use;
...
void SP_misc_slotmachine(gentity_t *ent){
	...
	//RoboPhred: I apparently removed this line or somthing like it.
	ent->genericValue6 = ent->count;
...
void G_PortalifyEntities(gentity_t *ent)
{
	...
	//RoboPhred: No more, we need to save since all map ents are copied  to the save file
	/*
...
void HolocronTouch(gentity_t *self, gentity_t *other, trace_t *trace)
{
	...
	//G_AddEvent( other, EV_ITEM_PICKUP, self->s.number ); //Lugormod
	...
	*/
	...
	//if (self->parent && self->parent->inuse) { //Lugormod
	//turn item cone off
	//}
...
void SP_misc_holocron(gentity_t *ent)
{
	...
	gentity_t *base;
	if (base = G_Spawn()) {
	base->classname = "misc_model_breakable";
	base->model = "models/map_objects/mp/holo.md3";
	VectorCopy (tr.endpos, base->s.origin);
	base->s.origin[2] += ent->r.mins[2];
	base->s.apos.trBase[2] = ent->s.apos.trBase[2];
	SP_misc_model_breakable(base);
	base->r.contents = 0;
...
void SP_misc_model_health_power_converter( gentity_t *ent )
{
	...
	//ent->s.maxhealth = ent->s.health = ent->count; //Lugormod this was commented out for some reason
	ent->s.shouldtarget = qtrue;
	ent->s.teamowner = 0;
	ent->s.owner = ENTITYNUM_NONE;
...
void Use_Target_Escapetrig( gentity_t *ent, gentity_t *other, gentity_t *activator )
{
	...
	else if (gEscaping)
	{
		...
		{ //all of the survivors get 100 points!
			...
			{
				AddScore(&g_entities[i], g_entities[i].client->ps.origin, ent->genericValue1);
			}
			i++;
		}
		if (activator && activator->inuse && activator->client)
		{ //the one who escaped gets 500
			AddScore(activator, activator->client->ps.origin, ent->genericValue2);
		}
...
void SP_target_escapetrig(gentity_t *ent)
{
	...
	//RoboPhred
	G_SpawnInt("survivePoints", "100", &ent->genericValue1);
	G_SpawnInt("excapePoints", "500", &ent->genericValue2);
