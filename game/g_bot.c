...
//RoboPhred: why is the above int static, but this one not?
int				g_numArenas;
...
int G_GetMapTypeBits(char *type)
{
	...
	if( *type ) {
		...
		if( strstr( type, "battleground" ) ) {
			typeBits |= (1 << GT_BATTLE_GROUND);
		}
		if( strstr( type, "ctf" ) ) {
			typeBits |= (1 << GT_CTF);
		}
        ...
        if( strstr(type, "reborn" )) {
			typeBits |= (1 << GT_REBORN);
        }
        if( strstr(type, "reborn" )) {
			typeBits |= (1 << GT_GHOST);
        }
...
*/
//Lugormod
//I'd like for this not to be random, but lowest score.
...
int G_RemoveRandomBot( int team ) {
	...
	for ( i=0 ; i< g_maxclients.integer ; i++ ) {
		...
		//Lugormod say bye
                ...
                //Com_Printf("info: %s say bye\n", netname);
                if (bs) {//Don't know how this could not be.
                        bs->chatObject = NULL;
                        bs->chatAltObject = NULL;
                        BotDoChat(bs, "Leaving", 1);
                        trap_EA_Say(i, bs->currentChat);
                }
                //trap_SendConsoleCommand( EXEC_INSERT, va("kick \"%s\"\n", netname) );
                //Lugormod leave instead of kicked
...
void G_CheckMinimumPlayers( void ) {
	...
	if (checkminimumplayers_time > level.time || level.startTime + 30000 > level.time) {
		return;
	}
	//only check once each 10 - 30 seconds
	checkminimumplayers_time = level.time + Q_irand(10000,30000);
...
//	Info_SetValueForKey( userinfo, "characterfile", Info_ValueForKey( botinfo, "aifile" ) );
    ...
    if (g_gametype.integer == GT_REBORN) { //Lugormod
	...
	//Lugormod moved greetings to here
    BotDoChat(botstates[clientNum], "GeneralGreetings", 0);
	...
	{
                //if (g_gametype.integer == GT_REBORN) { //Lugormod test
                //        SetTeam(bot,"blue");
                //}
                if( delay == 0 ) {
			ClientBegin( clientNum, qfalse );
...
static void G_LoadBots( void ) {
	...
	trap_Cvar_Register( &botsFile, "g_botsFile", "", CVAR_INIT );
	if( *botsFile.string ) {
