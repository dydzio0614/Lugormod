...
{
	gentity_t	*ent = GetEnt(entID);// = &g_entities[entID];
...
int	Q3_GetTag( int entID, const char *name, int lookup, vec3_t info )
{
	...
	if (!ent->inuse)
	{
		//RoboPhred: dont care, mp scripting is crippled
		//assert(0);
		return 0;
	}

	//RoboPhred: tags without owners are perfectly fine
	/*
	if(!ent->ownername)
		return 0;
	*/
...
{
	gentity_t	*teleEnt = GetEnt(entID);// = &g_entities[entID];
...
{
	gentity_t	*found = GetEnt(entID);// = &g_entities[entID];
...
static void SetLowerAnim( int entID, int animID)
{
	...
	G_SetAnim(ent,SETANIM_LEGS,animID,SETANIM_FLAG_RESTART|SETANIM_FLAG_HOLD|SETANIM_FLAG_OVERRIDE,0);
}
...
static void SetUpperAnim ( int entID, int animID)
{
	...
	G_SetAnim(ent,SETANIM_TORSO,animID,SETANIM_FLAG_RESTART|SETANIM_FLAG_HOLD|SETANIM_FLAG_OVERRIDE,0);
}
...
{
	gentity_t	*self = GetEnt(entID);//  = &g_entities[entID];
...
static void Q3_SetStartFrame( int entID, int startFrame )
{
	//G_DebugPrint( WL_WARNING, "Q3_SetStartFrame: NOT SUPPORTED IN MP\n");
        //Com_Printf("info: Q#_SetEndFrame\n");
        gentity_t *ent = &g_entities[entID];
...
============
*/
void animate_model(gentity_t *self); //Lugormod
static void Q3_SetEndFrame( int entID, int endFrame )
{
    gentity_t *ent = GetEnt(entID);// = &g_entities[entID];

    ent->genericValue5 = endFrame;
    if (!ent->think) {
...
//		//can never be set
	...
	case SET_HUD:
        ...
        case SET_RAILCENTERTRACKUNLOCKED:
                if (1) {
                        gentity_t *tEnt = G_PickTarget((char*)data);
                        if (tEnt && tEnt->inuse) {
                                tEnt->flags &= ~FL_INACTIVE;
                        }
                }
                return 0;
                break;
