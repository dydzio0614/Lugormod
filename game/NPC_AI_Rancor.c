...
void NPC_Rancor_Precache( void )
{
	...
	G_SoundIndex( "sound/chars/rancor/chomp.wav" );
        //G_SoundIndex( "sound/chars/rancor/rancor_roar_02" );
}
			...
			{//full eat
				...
				if ( NPC->activator->health > 0 && NPC->activator->client )
				{
					...
					{//no more thinking for you
						//RoboPhred: in death code.
						TossClientItems( NPC );
						NPC->activator->NPC->nextBStateThink = level.time + Q3_INFINITE; //Ufo: Q3_INFINITE is insufficient for leveltime dependant stuff
					}
				}
			}
		}
	...
	else if ( TIMER_Done2( NPC, "attack_dmg2", qtrue ) )
	{
		...
		case BOTH_ATTACK3:
			...
			{//swallow victim
				...
				// RoboPhred: was 'if', could cause players to invisible by "/team s; respawn" when grabbed by rancor.
				else if ( NPC->activator->client )
...
void NPC_BSRancor_Default( void )
{
	...
	else 
	{
		...
		{
			G_Sound( NPC, CHAN_AUTO, G_SoundIndex( va("sound/chars/rancor/snort_%d", Q_irand(1, 3))) );
