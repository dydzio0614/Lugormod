...
unsigned char G_ReadMD3MinsMaxes(const fileHandle_t fh, vec3_t mins, vec3_t maxs) {
	md3header_t header;
	md3boneFrame_t boneFrames;//[1024];
	trap_FS_Read((void*)&header,sizeof(md3header_t),fh);
	trap_FS_Read((void*)&boneFrames,
		sizeof(md3boneFrame_t)
		//* header.numBoneFrames
		,fh);
	mins[0] = boneFrames.mins[0];
	mins[1] = boneFrames.mins[1];
	mins[2] = boneFrames.mins[2];
	maxs[0] = boneFrames.maxs[0];
	maxs[1] = boneFrames.maxs[1];
	maxs[2] = boneFrames.maxs[2];
	return (unsigned char)header.numBoneFrames;
}
...
int G_ModelIndex( const char *name ) {
	...
	int i;
	//Lugormod
	if ((i = G_FindConfigstringIndex (name, CS_MODELS, MAX_MODELS, qfalse))
		&& model_frames[i] > 0){
			return i;
	}
	fileHandle_t fh = 0;
		...
		&& name[0] != '@') {
			...
			{ //try models/ then, this is assumed for registering models
				...
				if (!fh)
				{
					...
					//Lugormod shouldn't we be able to just return now?
					return 0;
				}
			}
	...
	i = G_FindConfigstringIndex (name, CS_MODELS, MAX_MODELS, qtrue);

	if (fh){
		model_frames[i] = G_ReadMD3MinsMaxes(fh, model_mins[i], model_maxs[i]);
...
int G_SoundIndex( const char *name ) {
	// RoboPhred: icarus precache on a $GET()$.  For the map "blueice nightfall".
	//assert(name && name[0]);
...
void G_CreateFakeClient(int entNum, gclient_t **cl)
{
	...
	*cl = gClPtrs[entNum];
	*/
	//Lugormod using new and improved G_Alloc ...
	//freing in G_FreeEntity in this file
	//BG_Alloc sux!
	*cl = (gclient_t*)G_Alloc(sizeof(gclient_t));
}
...
void GlobalUse(gentity_t *self, gentity_t *other, gentity_t *activator)
{
	...
	if (!self || ((self->flags & FL_INACTIVE) && Q_stricmp(self->classname, "lmd_gate") != 0))
		return;
...
void G_UseTargets2( gentity_t *ent, gentity_t *activator, const char *string ) {
	...
	while ( (t = G_Find (t, FOFS(targetname), string)) != NULL ) {
		if ( t == ent ) {
			...
			//RoboPhred: not needed, GlobalUse does this check
			//if ( t->use ) {
...
#define VM_OR_FINAL_BUILD
	...
	while (i < ENTITYNUM_MAX_NORMAL)
	{
		...
		if (ent->inuse)
		{
			...
			if (ent->target) {
				str = va("ENT %4i: Classname %s Target: %s\n", ent->s.number, className, ent->target);
			} else {
...
gentity_t *G_Spawn( void ) {
	...
	gentity_t	*e;
	//RoboPhred
	gentity_t *replaceCanidate = NULL;
	gentity_t *forceCanidate = NULL;

	e = NULL;	// shut up warning
	i = 0;		// shut up warning

	//RoboPhred: Look for free ents
	e = &g_entities[MAX_CLIENTS];
	for ( i = MAX_CLIENTS ; i<level.num_entities ; i++, e++) {

		if(replaceCanidate == NULL && Q_stricmp(e->classname, "tempEntity") == 0) {
			replaceCanidate = e;
		}
		...
		if(forceCanidate == NULL && e->freetime > level.startTime + 2000 && level.time - e->freetime < 1000){
			forceCanidate = e;
			continue;
		}
...
void G_FreeEntity( gentity_t *ed ) {
	...
	G_Free(ed->classname);
	G_Free(ed->fullName);
	G_Free(ed->targetname);
	G_Free(ed->target);
	G_Free(ed->target2);
	G_Free(ed->paintarget);
	G_Free(ed->NPC_targetname);
	G_Free(ed->NPC_type);
	G_Free(ed->client); //FreeFakeClient is called below
	G_Free(ed->model);
	G_Free(ed->model2);
	G_Free(ed->parms);
	G_Free(ed->spawnString);
	*/
	//now that I use the trap_RealAlloc function, this interferes and causes crashes.
	//G_Free(ed->client);
	G_Free(ed->NPC);
	int i;

	for (i = 0; i < NUM_BSETS; i++) {
		G_Free(ed->behaviorSet[i]);
	}
	...
	for (i = 0; fields[i].name; i++) {
		//RoboPhred
		//doesnt matter anyway, entity spawn only uses LSTRING, not GSTRING
		if(fields[i].type != F_LSTRING && fields[i].type != F_GSTRING){
		//if (fields[i].type != F_LSTRING) {
			continue;
		}
		//my free is safe, I don't need to validate the pointer.
		G_Free(*(char**)(b+fields[i].ofs));
	}
	...
	//RoboPhred
	if(ed->Lmd.spawnData){
		removeSpawnstring(ed->Lmd.spawnData);
	}

	for(i = 0;i < sizeof(ed->GenericStrings) / sizeof(ed->GenericStrings[0]); i++)
		G_Free(ed->GenericStrings[i]);
	...
	//RoboPhred: keep our number for logical ent position identification.
	number = ed->s.number;
...
===============
*/
void G_AddEvent( gentity_t *ent, int ev, int eventParm ) {
	int		bits;
	...
	if ( ent->client ) {
		...
		ent->client->ps.externalEvent = ev | bits;
...
void G_Sound( gentity_t *ent, int channel, int soundIndex ) {
	...
	{ //let the client remember the index of the player entity so he can kill the most recent sound on request
		...
		if (ent->client->ps.fd.killSoundEntIndex[channel-50] > MAX_CLIENTS &&
			&g_entities[ent->client->ps.fd.killSoundEntIndex[channel-50]] &&
...
qboolean TryHeal(gentity_t *ent, gentity_t *target)
{
	...
	{ //it's not dead yet...
		...
		{ //this thing can be healed by the class this player is using
			...
			{
				G_SetAnim( ent, SETANIM_TORSO, BOTH_BUTTON_HOLD, SETANIM_FLAG_OVERRIDE|SETANIM_FLAG_HOLD, 0 );
			}
...
//RoboPhred
extern vmCvar_t lmd_enableCorpseDrag;
void TryUse( gentity_t *ent )
{
	...
	if (g_gametype.integer == GT_SIEGE && !gSiegeRoundBegun)
	{ //nothing can be used til the round starts.
		return;
	}

#ifdef LMD_NEW_JETPACK
	//We are jetpacking, jump to the jetpack code.
	//Merc does not use this; they use the spacebar for jetpack and use for grapple.
	if(PlayerAcc_Prof_GetProfession(ent) != PROF_MERC && ent->client->jetPackTime > level.time) {
		goto tryJetPack;
	}
#endif
	...
	//Lugormod Don't use stuff in duels
	...
	if (trace.fraction == 1.0f && g_gametype.integer != GT_SIEGE && g_gametype.integer != GT_BATTLE_GROUND) //Ufo: fucks up tech healing on siege
	{
		goto tryJetPack;
	}
	...
	//RoboPhred
	if(lmd_enableCorpseDrag.integer) {
		...
		|| g_ff_objectives.integer) )
	{
		...
		*/
		...
		//RoboPhred
		ent->client->lastTargetUse = level.time;
		return;
	}
...
tryJetPack:
	...
	//RoboPhred: mercs use jetpack from spacebar
#ifdef LMD_NEW_JETPACK
	if(PlayerAcc_Prof_GetProfession(ent) == PROF_MERC)
		goto tryDispensors;
#endif

	//RoboPhred: dont run a jetpack or item drop unless we have not used an item.  This stops jetpack activating when
	//triggernig a useable teleporter, or dropping an item on a trigger_once use.
	if(ent->client->lastTargetUse + 800 > level.time)
		return;

	//if we got here, we didn't actually use anything else, so try to toggle jetpack if we are in the air, or if it is already on
	if (ent->client->ps.stats[STAT_HOLDABLE_ITEMS] & (1 << HI_JETPACK) && ent->client->ps.groundEntityNum == ENTITYNUM_NONE)
	{
		ItemUse_Jetpack(ent);
		return;
	}
...
tryDispensors:
	...
	else if ( (ent->client->ps.stats[STAT_HOLDABLE_ITEMS] & (1 << HI_HEALTHDISP)))
	{ //if you used nothing, then try spewing out some health
		trace_t trToss;
		vec3_t fAng;
		vec3_t fwd;
