...
void NPC_BSFollowLeader (void)
{
	...
	if ( !NPC->enemy  )
	{//no enemy, find one
		NPC_CheckEnemy( (qboolean)(NPCInfo->confusionTime<level.time), qfalse, qtrue );//don't find new enemy if this is tempbehav
		if ( NPC->enemy )
		{//just found one
	...
	else 
	{
		...
		{
			NPC_CheckEnemy((qboolean) (NPCInfo->confusionTime<level.time||NPCInfo->tempBehavior!=BS_FOLLOW_LEADER), qfalse, qtrue );//don't find new enemy if this is tempbehav
		}
	}
