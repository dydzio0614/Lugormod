...
//
//RoboPhred
#define LMD_DYNAMIC_NPCS
#ifdef LMD_DYNAMIC_NPCS
#define MAX_NPC_FILE_SIZE 0x20000
struct NPCParams_s{
	char *Params;
	int Size;
}NPCData;
#else
//RoboPhred: blindly increase, add a zero
...
//	const char	*filename = "ext_data/NPC2.cfg";
	...
	//RoboPhred
	char *npcExtensionListBuf = (char *)G_Alloc(MAX_NPC_FILE_SIZE); // The list of file names read in
	...
	fileCnt = trap_FS_GetFileList("ext_data/NPCs", ".npc", npcExtensionListBuf, MAX_NPC_FILE_SIZE);
	...
	for(i = 0;i<fileCnt; i++, holdChar += npcExtFNLen+1){
		...
		//Com_Printf("^5DEBUG: ^4NPC Parse ^3%s\n", holdChar);
		if(len < 0)
			continue;
		NPCData.Size += len + 1; //+1 for strcat of linefeed
		trap_FS_FCloseFile(f);
	}
#endif
	...
	//RoboPhred
#ifdef LMD_DYNAMIC_NPCS
	marker = NPCData.Params = (char *)G_Alloc(NPCData.Size);
#else
	marker = NPCParms;
#endif
	*marker = 0;
...
//		Com_Printf( "Parsing %s\n", holdChar );
		...
		if ( len == -1 )
			Com_Printf( "Error reading NPC file ext_data/NPCs/%s\n", holdChar);
		else
		{
#ifdef LMD_DYNAMIC_NPCS
			if(totallen + len >= NPCData.Size)
				G_Error("Lugormod NPC file size prediction failed!");
#else
			...
			//RoboPhred: use the safe cmd
			Q_strncpyz(marker, va("%s\n", npcParseBuffer), NPCData.Size - totallen);
			len++;
			...
			totallen += len;
#ifdef LMD_DYNAMIC_NPCS
			marker = NPCData.Params + totallen;
#else
