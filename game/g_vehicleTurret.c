...
//-----------------------------------------------------
{
	...
	//float	aimCorrect = qfalse;
        ...
        gentity_t *passenger = (gentity_t*)pVeh->m_ppPassengers[turretStats->passengerNum-1]; //Lugormod
	...
	AnglesSubtract( curAngles, pVeh->m_vOrientation, curAngles );
        if (turretStats->invPitchAxis){
                curAngles[turretStats->pitchAxis] *= -1;
        }
        
        if (turretStats->invYawAxis) {
                curAngles[turretStats->yawAxis] *= -1;
        }
	...
	//Now clamp the desired relative angles
        float cyl = pVeh->m_pVehicleInfo->turret[turretNum].yawClampLeft;
        float cyr = pVeh->m_pVehicleInfo->turret[turretNum].yawClampRight;
        float cpu = pVeh->m_pVehicleInfo->turret[turretNum].pitchClampUp;
        float cpd = pVeh->m_pVehicleInfo->turret[turretNum].pitchClampDown;
        
	//clamp yaw
	desiredAngles[YAW] = AngleNormalize180( desiredAngles[YAW] );
	if ( (cyl || cyr) && desiredAngles[YAW] > cyl)
	{
		aimCorrect = qfalse;
		desiredAngles[YAW] = cyl;
	}
	if ( (cyr || cyl) && desiredAngles[YAW] < cyr )
	{
		aimCorrect = qfalse;
		desiredAngles[YAW] = cyr;
	}
	//clamp pitch
	desiredAngles[PITCH] = AngleNormalize180( desiredAngles[PITCH] );
	if ( (cpd || cpu )&& desiredAngles[PITCH] > cpd)
	{
		aimCorrect = qfalse;
		desiredAngles[PITCH] = cpd;
	}
	if ( (cpd || cpu ) && desiredAngles[PITCH] < cpu )
	{
		aimCorrect = qfalse;
		desiredAngles[PITCH] = cpu;
	}
	...
	if ( turretStats->pitchBone )
	{
		...
		//pitchAngles[turretStats->pitchAxis] = desiredAngles[PITCH];
...
//-----------------------------------------------------
{
	...
	count = G_RadiusList( org2, turretStats->fAIRange, parent, qtrue, entity_list, qfalse );
...
//-----------------------------------------------------
{
	...
	if ( turretStats->passengerNum 
             ...
             //&& pVeh->m_ppPassengers[turretStats->passengerNum]
                )
