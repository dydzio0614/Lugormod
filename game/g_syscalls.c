...
void	trap_Argv( int n, char *buffer, int bufferLength ) {
	...
	//RoboPhred: filter out linefeeds.  Ravensoft assumed this was done, but it isn't!
	//This can mess with chat, and send rcon cmds when doine with callvote.
	/*
	char *ln;
	while((ln = strchr(buffer, '\n')))
		ln[0] = 'n';
	while((ln = strchr(buffer, '\r')))
		ln[0] = 'r';
	*/
	int i, len = strlen(buffer);
	for(i = 0; i < len; i++){
		//>0 needed since accented letters are beyond the 127 boundary and wrap around to negitives.
...
void trap_SetBrushModel( gentity_t *ent, const char *name ) {
	...
	if(!name || (name[0] != '*' && name[0] != '#')) {
		Com_Printf("^1trap_SetBrushModel called with invalid bmodel.\n");
		return;
	}
	syscall( G_SET_BRUSH_MODEL, ent, name );
	//RoboPhred
	if(!ent->targetname || !ent->targetname[0]){
		if(ent->targetname) //have an empty targetname
